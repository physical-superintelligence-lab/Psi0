...
def task_description_from_foler_name(folder_name: str) -> str:
    # Example: "navigate_to_target" -> "Navigate to target"
...
class HE2LeRobotConverter:
    ...
    def build_obs(self, 
                #   prev_rpy_height:Dict[str, Any], 
                  frame: Dict[str, Any], depth_arr: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "states": frame["states"]["qpos"] + frame["states"]["hand_joints"]
        }
    ...
    def build_act(self, frame: Dict[str, Any]) -> List[float]:
        ...
        # frame["actions"]["qpos"] is decoded from token
        return frame["actions"]["token"] + frame["actions"]["hand_joints"]
    ...
    def make_one_episode(
        ...
        try:
            ...
            for i, (frame, depth_arr) in enumerate(zip(data_list, iter_depths())):
                obs = self.build_obs( frame, depth_arr)  #prev_rpy_height,
            ...
            def frame_iter():
                for i, p in enumerate(rgb_paths):
                    if p is None:
                        raise ValueError(f"Frame {i} in episode {episode_index} has no image path")
                    yield iio.imread(p)
    ...
    def run(self, data_root: Path, work_dir: Path, chunks_size: int, num_workers: int, robot_type: str, task:str|None=None):
        ...
        self.task_description_dict = json.load(open(tdd)) if tdd.is_file() else {}
        ...
        for task_name, task_dir, cat_name, leaf_name in iter_tasks(data_root, tasks=[task] if task else []):
            ...
            print("="*40 + "\n")
            desc = self.task_description_dict.get(leaf_name, task_description_from_foler_name(leaf_name))
        ...
        print("Done. Try loading existing progress...")
