...
# simple-24-v20 + bounds_q99 normalization. Otherwise identical to
# finetune-simple-psi0-mix33.sh, which trained 100k steps clean (best eval 0.223).
# Note: 934,559 frames here vs 1,329,704 for mix33, so 100k steps is 27.4 epochs
# rather than 19.3 -- use ~70k if you want matched data exposure instead.
if [ "$#" -lt 1 ]; then
    echo "Usage: $0 [task] [exp]"
    echo "Example: $0 simple-24-v20 mix24-q99-100k"
    exit 1
fi

export task="${1:-simple-24-v20}"
...
--exp=$exp \
--train.name=finetune \
--train.resume_from_checkpoint=.runs/finetune/mix24-q99-100k.simple.flow1000.cosine.lr1.0e-04.b256.gpus8.2608090753 \
...
--train.max_training_steps=150000 \
...
--train.lr_scheduler_kwargs.weight_decay=0.01 \
--train.lr_scheduler_kwargs.eps=1e-6 \
--train.lr_scheduler_kwargs.betas 0.95 0.999 \
--log.report_to=wandb \
--data.root_dir=/hfm/data/ \
