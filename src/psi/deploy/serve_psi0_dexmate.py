...
class Server:
    ...
    def _parse_obs_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        head_joints = state_dict["head_joints"].copy() # shape (3,)
        torso_joints = state_dict["torso_joints"].copy() # shape (3,)

        obs = np.concatenate([hand_joints, arm_joints, head_joints, torso_joints], axis=-1) # (32,)
