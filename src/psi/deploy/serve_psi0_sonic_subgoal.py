...
class RealTimeChunkController:
    ...
    def _predict_action_rtc(self, o, A_prev, d):
        assert 'goal_imgs' in o and o['goal_imgs'] is not None, \
            "goal_imgs missing from obs dict; subimg server requires a goal image at every step"
    ...
    def _parse_obs_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for cam_idx, img_key in enumerate(self.launch_cfg.data.transform.repack.image_keys):
            if img_key not in image_dict:
                raise ValueError(
                    f"Required observation image '{img_key}' missing from client payload; "
                    f"got keys {list(image_dict.keys())}"
                )
            imgs[f"cam{cam_idx}"] = Image.fromarray(np.clip(image_dict[img_key], 0, 255).astype(np.uint8))

        # subimg server requires a goal image; fail loudly if config or payload is missing it
        goal_image_key = self.launch_cfg.data.transform.repack.goal_image_key
        if goal_image_key is None:
            raise RuntimeError(
                "launch_cfg.data.transform.repack.goal_image_key is None; "
                "this server only serves models trained with a subgoal image"
            )
        if goal_image_key not in image_dict:
            raise ValueError(
                f"Required goal image '{goal_image_key}' missing from client payload; "
                f"got keys {list(image_dict.keys())}"
            )
        goal_pil = Image.fromarray(np.clip(image_dict[goal_image_key], 0, 255).astype(np.uint8))
        goal_processed = self._process_img(goal_pil) # [transformed_img]
        batch_goal_images = [goal_processed[0]] # batch size == 1
