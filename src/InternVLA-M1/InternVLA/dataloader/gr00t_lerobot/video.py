...
def get_frames_by_timestamps(
    ...
    if video_backend == "decord":
        ...
        assert step_indices_for_assert is None or np.array_equal(indices, step_indices_for_assert), f"indices {indices} do not match step_indices_for_assert {step_indices_for_assert}"
        frames = vr.get_batch(indices)
        return frames.asnumpy()
    elif video_backend == "opencv":
        # Open the video file
        ...
        tmp = frames.transpose(0, 2, 3, 1)
        import time  # save image with timestamp named image_0_timestamp.jpg
        import os
        os.makedirs("/hfm/boqian/tmp/test_video", exist_ok=True)
        image_path = os.path.join("/hfm/boqian/tmp/test_video", f"image_{current_ts}_{time.time()}.jpg")
        cv2.imwrite(image_path, tmp[0])
        return tmp
    else:
        raise NotImplementedError
