...
def calculate_dataset_statistics(parquet_paths: list[Path]) -> dict:
    ...
    for le_modality in all_low_dim_data.columns:
        if le_modality not in ["states", "action"]:
            continue
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        if preload_all:
            self._preload_all_trajectories()
    ...
    def _get_all_steps_single_process(self) -> list[tuple[int, int]]:
        ...
        for trajectory_id, trajectory_length in tqdm(zip(self.trajectory_ids, self.trajectory_lengths), total=len(self.trajectory_ids), desc="Getting All Step"):
            ...
            self.delete_pause_frame = False # FIXME
    ...
    def _preload_all_trajectories(self):
        """预加载所有trajectories到内存"""
        print("Preloading all trajectories...")
        self._trajectory_cache = {}
        for trajectory_id in tqdm(self.trajectory_ids, desc="Loading trajectories"):
            chunk_index = self.get_episode_chunk(trajectory_id)
            parquet_path = self.dataset_path / self.data_path_pattern.format(
                episode_chunk=chunk_index, episode_index=trajectory_id
            )
            self._trajectory_cache[trajectory_id] = pd.read_parquet(parquet_path)
        print(f"Preloaded {len(self._trajectory_cache)} trajectories")
    ...
    def get_video(
        ...
        return get_frames_by_timestamps(
            ...
            step_indices_for_assert=step_indices,
        )
    ...
    def get_state_or_action(
        ...
        return self.retrieve_data_and_pad(
            ...
            # padding_strategy="first_last",           # HACK for realdata
        )
    ...
    def get_language(
        ...
        return self.tasks.loc[task_indices]["description"].tolist() # 适配zhenyu脚本
...
class LeRobotMixtureDataset(Dataset):
    ...
    def sample_step_with_shuffle_is_true(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        """Sample a single step from the dataset."""
        '''only use this when DataLoader is initialized with shuffle=True'''
        print(index)
        # return self.sampled_steps[index]
        ...
        # base_index = rng.choice(dataset.trajectory_lengths[trajectory_index])
        # return dataset, trajectory_id, base_index
        # no random, follow the order of the dataset
        # single_step_index = rng.choice(len(dataset.all_steps))
        # trajectory_id, base_index = dataset.all_steps[single_step_index]
        ...
        for attempt in range(max_retries):
            try:
                ...
                for video_key in dataset.modality_keys["video"]:
                    ...
                    image = Image.fromarray(image).resize((224, 224))
                    # # use time stamp as filename
                    # import time
                    # filename = f"/hfm/boqian/storage/tmp/image_{int(time.time())}_{index}.png"
                    # image.save(filename)
                    images.append(image)
