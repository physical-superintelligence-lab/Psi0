...
class Gr00tN1d6ActionHead(nn.Module):
    ...
    def get_action_with_features_with_rtc(
        self,
        backbone_features: torch.Tensor,
        state_features: torch.Tensor,
        embodiment_id: torch.Tensor,
        ...
        with torch.no_grad():
            ...
            # Run denoising steps.
            for t in range(self.num_inference_timesteps):
                t_cont_float = t / float(self.num_inference_timesteps)  # e.g. goes 0, 1/N, 2/N, ...
                t_cont = torch.tensor(t_cont_float, dtype=torch.float32, device=device)  # 转换为 tensor
                t_discretized = int(t_cont_float * self.num_timestep_buckets)
                ...
                with torch.enable_grad():
                    ...
                    action_features = self.action_encoder(actions_grad, timesteps_tensor, embodiment_id)
                    # Add position embedding.
                    if self.config.add_pos_embed:
                        pos_ids = torch.arange(action_features.shape[1], dtype=torch.long, device=device)
                        pos_embs = self.position_embedding(pos_ids).unsqueeze(0)
                    ...
                    # 1. 外推干净data X_1
                    pred_x1 = (1 - t_cont) * pred_velocity + actions_grad # (1, 50, 128)
                
                    # 处理 prev_actions 和 mask_expanded: 从 (1, 30, 36) 转换为 (1, 50, 128)
                    target_horizon = pred_x1.shape[1]  # 50
                    target_dim = pred_x1.shape[2]      # 128
                    
                    # 对 prev_actions 进行 padding
                    prev_actions_padded = torch.nn.functional.pad(
                        prev_actions, 
                        (0, target_dim - prev_actions.shape[2], 0, target_horizon - prev_actions.shape[1], 0, 0),
                        mode='constant', 
                        value=0
                    )  # (1, 50, 128)
                    
                    # 对 mask_expanded 进行 padding
                    mask_expanded_padded = torch.nn.functional.pad(
                        mask_expanded,
                        (0, target_dim - mask_expanded.shape[2], 0, target_horizon - mask_expanded.shape[1], 0, 0),
                        mode='constant',
                        value=0
                    )  # (1, 50, 128)

                    # 2. 计算(Y - pred_X_1) * weights
                    # error = (prev_actions - pred_x1.detach()) * mask_expanded
                    error = (prev_actions_padded - pred_x1.detach()) * mask_expanded_padded

                    # 3. vjp
                    pinv_correction = torch.autograd.grad(
                        outputs=pred_x1,  # [1,T,D]
                        inputs=actions_grad,        # [1,T,D]
    ...
    def get_action_with_rtc(self, 
        ...
        ## RTC related ##
        H = prev_actions.shape[1]
        D = prev_actions.shape[2]
        ...
        mask_expanded = mask.view(1, -1, 1).expand(1, H, D)
        ##              ##

        return self.get_action_with_features_with_rtc(
            backbone_features=features.backbone_features,
            state_features=features.state_features,
            embodiment_id=action_input.embodiment_id,
            backbone_output=backbone_output,
...
class Gr00tN1d6(PreTrainedModel):
    ...
    def get_action_with_rtc(self, 
        ...
        # Forward through backbone
        backbone_outputs = self.backbone(backbone_inputs)
        action_outputs = self.action_head.get_action_with_rtc(backbone_outputs, action_inputs, prev_actions, inference_delay, execution_horizon, mask_schedule, guidance_weight)
